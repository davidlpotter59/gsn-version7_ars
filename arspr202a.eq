/*  arspr202a.eq
    ...
    a starting and ending date range - selected agent only
*/

description 
Select an Agent to list payments received by Agent and Policy number with a date range ;

include "startend.inc"
                    
define unsigned ascii number l_agent_no=parameter/prompt="Enter Agent Number "

define file sfsagent1 = access sfsagent, set
sfsagent:company_id = arschksu:company_id,
...
where arschksu:trans_date >= l_starting_date and 
      ...
      sfpname:agent_no     = l_agent_no and 
...
sfpname:name[1]/width=30/noheading/column=15
