/*  arspr800na
    ...
    Not Archived Report to list in summary arsbilling records - processed and run from arsin042 only 
*/

description 
NON Archive Version List, in summary, arsbilling records - select a starting and ending date that have a balance as of the date range entered ;

include "ending.inc"
define string l_prog_number = "ARSPR800na - Version 7.10"

define signed ascii number l_nsf_51 = if arsbilling:trans_code one of 51 then 
arsbilling:total_amount_paid else 0 

define signed ascii number l_billed_amount = if arsbilling:status not one of "O" then arsbilling:installment_amount 
else 0.00
...
--and arsbilling:policy_no = 600000984
